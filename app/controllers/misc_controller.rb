class MiscController < ApplicationController
  def print
    ...
    @tokens = Token.where.not(course: 'JLPT').joins(:token_order).order('tokens.course, tokens.number, token_orders.weight, tokens.created_at')
    @jlpt_n5 = Token.where(course: 'JLPT').joins(:token_order).order('tokens.course, tokens.number, token_orders.weight, tokens.created_at')
  end
end
