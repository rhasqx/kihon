class Token < ApplicationRecord
  ...
  has_one :token_order, foreign_key: :name, primary_key: :pos
  ...
  def self.search(search)
    if search
      where("lower(german) LIKE :search or lower(english) LIKE :search or lower(romaji) LIKE :search or lower(kanji) LIKE :search or lower(hiragana) LIKE :search or lower(katakana) LIKE :search", search: "%#{search.downcase}%")
    else
      all
    end
  end
